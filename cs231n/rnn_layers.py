...
def rnn_backward(dh, cache):
    ...
    for step in range(T-1, -1, -1):
        dh_current = dh[:,step,:] + dprev
        dx_t, dprev, dWx_t, dWh_t, db_t = (rnn_step_backward(dh_current, cache[step]))
        dx[:,step,:] += dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def lstm_backward(dh, cache):
    ...
    for step in range(T-1, -1, -1):
        dh_current = dh[:,step,:] + dprev
        dx_t, dprev, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dh_current, dprev_c, cache[step])
        dx[:,step,:] += dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
